			...
			if(move_uploaded_file($_FILES['file1']['tmp_name'],$filetujuan1))
			{
				...
				foreach($sheets as $sheet)
				{
					...
					while ( $i <= $countrow )
					{
						...
						$distribusi =  $sheetData[$i]["G"];		
						$segmen = '';
						$grossweight = $sheetData[$i]["I"];
						$netweight = $sheetData[$i]["H"];
						...
						*/
						...
						//echo $productid . "/" . $rubberid . "/" . $configid . "/" . $item  . "<br>";
						...
						$selisih = 0;
						//$distribusi = '';
						$harga = $sheetData[$i]["L"] * 10;
						$hargam = $sheetData[$i]["M"] * 10;
						$hargar = $sheetData[$i]["N"] * 10;
						$kelas = '01';
						$kadar = 75.5;
						$tukarb = 84.5;
						$tukarj = 0;
						...
						if ($productid <> '')
						{
							$tsqlA = " 	insert into dbburungfrank.dbo.t_stockinv5 (m_cabang, m_kodebarang, m_lokasi, m_productid, m_qty, m_harga, m_status) 
										values ('".$cabang."','".$kodebarang."','".$lokasi."','".$productid."', ".$qty.", ".$harga.",'".$status."')";
							$stmtA = sqlsrv_query($con_dbnew, $tsqlA);
					
							$tsqlB = " 	insert into dbburungfrank.dbo.t_stockdata5 (m_kodebarang, m_productid, m_hpp, m_configid, m_rubberid, m_item, m_klasifikasi
