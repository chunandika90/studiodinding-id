...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        <title>Report Receive Inv.</title>
        ...
        <table id="table1" class="controller table table-bordered table-striped table-hover table-condensed">
        	<thead>
                <tr data-level="header" class="header">
                    <th width="20%"><div align="center">Report Receive Inv.</div></th>
            ...
            {	
                ...
                <tr <?php echo $tampilanfont; ?> data-level="<?php echo $row['vf_level']; ?>" id="rowke<?php echo $i; ?>" height="25px">
                    ...
                    <td>
                    	<div align="center">
                        <img src="images/view_icon.gif" style="cursor:pointer" id="viewdetail" onClick="oc_detail('<?php echo $prm; ?>','<?php echo $tanggal1; ?>','<?php echo $tanggal2; ?>','<?php echo $row['vf_lokasi2']; ?>','<?php echo $row['vf_idgroup']; ?>','<?php echo $row['vf_idkatg']; ?>','<?php echo $row['vf_iditem']; ?>','<?php echo $row['vf_idcab']; ?>','<?php echo $kdstock; ?>')"/>
                        </div>
                    </td>
                </tr>
                <?php
    ...
    <script type="text/javascript">
		...
		function oc_report(vparam)
		{
			...
			window.open("report-ttb.php?tg1="+base64_encode(tgl1)+"&tg2="+base64_encode(tgl2)+"&cb="+base64_encode(data)+'&gr='+base64_encode(kdgroup)+'&kt='+base64_encode(kdkatg)+'&it='+base64_encode(kditem)+'&prm='+base64_encode(vparam)+'&kdst='+base64_encode(kdstock),'_self');
		}

		function oc_detail(vparam,tgl1,tgl2,lokasi,kdgroup,kdkatg,kditem,kdcabang,kdstock)
		{
			var data={tg1:tgl1,tg2:tgl2,lok:lokasi,gr:kdgroup,kt:kdkatg,it:kditem,cb:kdcabang,kdst:kdstock,prm:vparam};
			var fungsi=function(respon){
					$("#listplu").html(respon);
				};
			$.get('report-ttb2.php',data,fungsi);
		}
		...
		{	
			window.open('report-ttbp.php?tg1='+tgl1+'&tg2='+tgl2+'&cb='+kdcab+'&gr='+kdgroup+'&kt='+kdkatg+'&it='+kditem+'&st='+kdstock,'_blank');
		}
		
		function exel1b (tgl1,tgl2,kdcab,kdgroup,kdkatg,kditem,kdstock)
		{	
			window.open('report-ttbx.php?tg1='+tgl1+'&tg2='+tgl2+'&cb='+kdcab+'&gr='+kdgroup+'&kt='+kdkatg+'&it='+kditem+'&st='+kdstock,'_blank');
		}
		
		function cetak1c (tgl1,tgl2,lokasi,kdcab,kdgroup,kdkatg,kditem,kdstock)
		{	
			window.open('report-ttb2p.php?tg1='+tgl1+'&tg2='+tgl2+'&lok='+lokasi+'&cb='+kdcab+'&gr='+kdgroup+'&kt='+kdkatg+'&it='+kditem+'&st='+kdstock,'_blank');
		}
		
		function exel1c (tgl1,tgl2,lokasi,kdcab,kdgroup,kdkatg,kditem,kdstock)
		{	
			window.open('report-ttb2x.php?tg1='+tgl1+'&tg2='+tgl2+'&lok='+lokasi+'&cb='+kdcab+'&gr='+kdgroup+'&kt='+kdkatg+'&it='+kditem+'&st='+kdstock,'_blank');
		}
