...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        <title>Survey(Checklist Proyek)</title>
    ...
    <div class="container" style="width:auto; padding:0 20px;">
	  ...
	  ">
		...
		<div class="input-group" style="display:flex; align-items:center; flex-wrap:wrap; gap:5px;">
		  <input type="text" class="input-large search-query" id="inputText" placeholder="Search Text" value="" style="flex:1; min-width:120px;">
		  ...
		  <?php if (substr($xparam[3],0,1) == 'Y') { ?>
			<button class="btn btn-success" onClick="edit_data('<?php echo $prm; ?>','','')">Input Checklist</button>
		  <?php } ?>
		</div>
	  </div>
	</div>
    ...
    <script type="text/javascript">
		...
		function oc_pos(vparam,vnomor)
		{
			...
			$.get('t_survey-list.php',data,fungsi);
		}
		...
		function oc_detail(vparam,nomor)
		{
			...
			$.get('t_survey-view.php',data,fungsi);
		}
		...
		function add_inv(vparam, nomor) {
			...
			$.get('t_survey-invoice.php', data, function(respon){
		...
		function hapus_data(vparam,nomor)
		{
			...
			$.get('t_survey-hapus.php',data,fungsi);
		...
		function edit_data(vparam,nomor)
		{
			...
			window.open("t_survey-edit.php?nm="+btoa(nomor)+'&pr='+btoa(pr)+'&prm='+btoa(vparam),'_self');
		}

		function print_all(nomor)
		{
			window.open("t_survey-printall.php?nm="+btoa(nomor),'_blank');
		}

		function print_data(kdcab,nomor,kdbrg,productid)
		{
			window.open("t_survey-print.php?nm="+btoa(nomor)+'&kdbrg='+btoa(kdbrg)+'&productid='+btoa(productid),'_blank');
		}
		...
		function batal_pos(vparam,nomor)
		{
			...
			$.get('t_survey-batal.php',data,fungsi);
		...
		function approve_po(vparam,nomor)
		{
			...
			$.get('t_survey-approve.php',data,fungsi);
			
			$('#view_modal').modal();
		}
		
		
		function add_checklist(vparam,nomor,no)
		{
			var data={nm:nomor,prm:vparam,no:no};
			var fungsi=function(respon){
					$("#viewdata").html(respon);
					oc_detail(vparam,nomor);
				};
			$.get('t_survey-checklist.php',data,fungsi);
		...
		function print_cert(nomor,kdbrg,productid)
		{
			...
			window.open("t_survey-sertifikat.php?nm="+btoa(nomor)+'&kdbrg='+btoa(kdbrg)+'&productid='+btoa(productid),'_blank');
		}
