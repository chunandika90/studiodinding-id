			...
			if(move_uploaded_file($_FILES['file1']['tmp_name'],$filetujuan1))
			{
				...
				foreach($sheets as $sheet)
				{
					...
					while ( $i <= $countrow )
					{
						...
						$qty = str_replace(",","",$sheetData[$i]["B"]);
						$rubberid = $sheetData[$i]["C"] ;
						$item = $sheetData[$i]["D"] ;
						$klasifikasi = $sheetData[$i]["E"] ;
						$kategori = $sheetData[$i]["F"] ;
						$segmen = $sheetData[$i]["G"] ;
						$grossweight = str_replace(",","",$sheetData[$i]["H"]);
						$netweight = str_replace(",","",$sheetData[$i]["I"]);
						$totbutir = $sheetData[$i]["J"] ;
						$totcarat = $sheetData[$i]["K"] ;
						$distribusi = $sheetData[$i]["L"] ;
						$harga = $sheetData[$i]["M"] ;
						$hargam = str_replace(",","",$sheetData[$i]["N"]);
						$hargar = str_replace(",","",$sheetData[$i]["O"]);
						$kelas = $sheetData[$i]["P"] ;
						$framecolor= $sheetData[$i]["Q"] ;
						$konstruksi = $sheetData[$i]["R"] ;
						...
						if ($productid <> '')
						{
							$tsqlA = " 	insert into dbburungmondial.dbo.t_stockinv (m_cabang, m_kodebarang, m_lokasi, m_productid, m_qty, m_harga, m_status) 
										values ('".$cabang."','".$kodebarang."','".$lokasi."','".$productid."', ".$qty.", ".$harga.",'".$status."')";
							$stmtA = sqlsrv_query($con_dbnew, $tsqlA);
					
							$tsqlB = " 	insert into dbburungmondial.dbo.t_stockdata (m_kodebarang, m_productid, m_hpp, m_configid, m_rubberid, m_item, m_klasifikasi
