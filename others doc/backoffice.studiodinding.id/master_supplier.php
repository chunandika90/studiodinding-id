...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <div class="container-fluid" padding: 0 10px;">
		<div style="overflow:auto; overflow-x:auto; height:800px; width:80%;">
            <span id="listuser">
			<?php
			if (substr($xparam[4],0,1) == 'Y')
			{
				?>
				<button class="btn" onClick="edit_modal('<?php echo $prm ; ?>','')">Tambah Supplier</button>
				<?php
			}
			?></th>
            <table class="table table-bordered table-striped table-hover table-condensed">
                <thead>
                    <tr>
                        <th colspan="4"><h4>Master Supplier</h4></th>
                    </tr>
                    ...
                    <?php
                        ...
                        while($row2 = $stmt2->fetch_assoc())
                        {
                            ...
                            <tr>
                                ...
                                <td onClick="view_modal('<?php echo $prm ; ?>','<?php echo $row2['m_kode'] ; ?>')" style="cursor:pointer"><?php echo $row2['contact_person']; ?></td>
                                <td onClick="view_modal('<?php echo $prm ; ?>','<?php echo $row2['m_kode'] ; ?>')" style="cursor:pointer"><?php echo $row2['nomor_telepon']; ?></td>
                                <td onClick="view_modal('<?php echo $prm ; ?>','<?php echo $row2['m_kode'] ; ?>')" style="cursor:pointer"><?php echo $row2['alamat']; ?></td>
                                <td onClick="view_modal('<?php echo $prm ; ?>','<?php echo $row2['m_kode'] ; ?>')" style="cursor:pointer"><?php echo $row2['bank_rekening'] ." - ". $row2['nomor_rekening']; ?></td>
    ...
    <script type="text/javascript">
		...
		function oc_type(vparam)
		{
			...
			window.open('master_supplier.php?kd='+base64_encode(vkode)+'&pr='+base64_encode(vperiod)+'&prm='+base64_encode(vparam),'_self');			
		}
		...
		function view_modal(vparam,vkode)
		{
			...
			$.get('master_supplier-view.php',data,fungsi);
		...
		function edit_modal(vparam,vkode)
		{
			...
			$.get('master_supplier-edit.php',data,fungsi);
		...
		function hapus_modal(vparam,vkode)
		{
			...
			$.get('master_supplier-hapus.php',data,fungsi);
		}
