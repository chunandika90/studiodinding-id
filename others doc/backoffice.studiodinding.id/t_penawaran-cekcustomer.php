<?php
	...
	if ( $sctx != '' )
	{
		$tsql = "select a.m_kode, a.m_nama, a.m_alamat, a.m_kota, a.m_telepon1, a.m_telepon2 
				 from mscustomer a where a.m_nama like '%".$sctx."%'" ;
    ...
    <tbody>
        <?php
            while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC))
            {
                ?>
                <tr>
                    <td onClick="selectcust('<?php echo $row['m_kode']; ?>','<?php echo $row['m_nama']; ?>','<?php echo $row['m_alamat']; ?>','<?php echo $row['m_kota']; ?>','<?php echo $row['m_telepon1']; ?>','<?php echo $row['m_telepon2']; ?>')" style="cursor:pointer"><?php echo $row['m_kode']; ?></td>
