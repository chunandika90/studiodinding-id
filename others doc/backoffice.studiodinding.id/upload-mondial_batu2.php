			...
			if(move_uploaded_file($_FILES['file1']['tmp_name'],$filetujuan1))
			{
				...
				foreach($sheets as $sheet)
				{
					...
					while ( $i <= $countrow )
					{
						...
						$parcel = $sheetData[$i]["D"] ;
						$butir = str_replace(",","",$sheetData[$i]["C"]);
						$carat = str_replace(",","",$sheetData[$i]["E"]);
						$sertifikat = $sheetData[$i]["F"] ;
						
						
						//$butir = $sheetData[$i]["C"] ;
						//$carat = $sheetData[$i]["D"] ;
						//$clarity = $sheetData[$i]["E"] ;
						//$class = $sheetData[$i]["F"] ;
						//$colour = $sheetData[$i]["G"] ;
						//$shape = $sheetData[$i]["H"] ;
						//$brand = $sheetData[$i]["I"] ;
						
						//echo $productid ."/".$parcel ."/".$butir ."/".$carat ."/".$sertifikat ."/". "<br>";
						...
						if ($productid <> '')
						{
							$tsqlA = " 	insert into dbburungfrank.dbo.t_stockdetail5(m_kodebarang, m_productid, m_configid, m_parcel,m_butir , m_Carat ,m_sertifikat) 
							            values ('J0000001','".$productid."','','".$parcel."','".$butir."','".$carat."','".$sertifikat."')";	
							//echo $tsqlA. "<br>";;		
