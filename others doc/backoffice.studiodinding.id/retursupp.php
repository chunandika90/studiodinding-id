...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <div class="container" style="width: auto; padding: 0 20px;">
        ...
        <div class="span3 input-prepend">
            ...
            <select name="periode" id="periode" class="input-medium">
                <?php
				$tsqlbulan = "select distinct LEFT(convert(varchar(10),m_tanggal,120),7) as co_periode from t_retursupp where m_status = 'A'" ;
    ...
    <script type="text/javascript">
		function oc_pos(vparam,vnomor)
		{
			...
			$.get('retursupp-list.php',data,fungsi);
		}
		...
		function oc_detail(vparam,kdcab,nomor)
		{
			...
			$.get('retursupp-view.php',data,fungsi);
		}
		...
		function add_inv(vparam,kdcab,nomor)
		{
			...
			$.get('retursupp-invoice.php',data,fungsi);
		...
		function hapus_data(vparam,kdcab,nomor)
		{
			...
			$.get('retursupp-hapus.php',data,fungsi);
		...
		{
			var pr = $('#periode').val() ;
			window.open("retursupp-edit.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&pr='+base64_encode(pr)+'&prm='+base64_encode(vparam),'_self');
		}

		function print_all(kdcab,nomor)
		{
			window.open("retursupp-printall.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor),'_blank');
		}

		function print_data(kdcab,nomor,kdbrg,productid)
		{
			window.open("retursupp-print.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&kdbrg='+base64_encode(kdbrg)+'&productid='+base64_encode(productid),'_blank');
		}
		...
		function batal_pos(vparam, kdcab,nomor)
		{
			...
			$.get('retursupp-batal.php',data,fungsi);
		...
		function print_cert(kdcab,nomor,kdbrg,productid)
		{
			...
			window.open("retursupp-sertifikat.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&kdbrg='+base64_encode(kdbrg)+'&productid='+base64_encode(productid),'_blank');
		}
