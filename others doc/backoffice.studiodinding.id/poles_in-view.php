    ...
    <tbody>
        <?php
            ...
            $tsql2 = "	select 	a.*, d.m_nama as m_namabarang
						...
						left join mslokasi c on a.m_lokasi = c.m_nama
...
<h5>Info Barang Asal</h5>
<table class="table table-bordered table-striped table-hover table-condensed">
    <thead>
        <tr>
            <th width="2%">No</th>
            <th width="10%">Nodoc</th>
            <th width="10%">Kode Barang</th>
            <th width="10%"><div align="right">Total Pcs</div></th>
            <th width="10%"><div align="right">Total Berat</div></th>
        </tr>
    ...
    <tbody>
        <?php
            ...
            $tqty = 0 ;
			$tgross = 0;
            $tsql3 = "	select 	a.*, d.m_nama as m_namabarang
                        from 	t_barang_in3 a
						left join mslokasi c on a.m_lokasi = c.m_nama
						left join msmaster d on d.m_type = 'MATERIAL' and a.m_kodebarang = d.m_kode
                        where 	a.m_lokasi = '".$row['m_lokasi']."' and 
            ...
            while( $row3 = sqlsrv_fetch_array( $stmt3, SQLSRV_FETCH_ASSOC))
            {
				...
				$i = $i + 1 ;
                $tqty = $tqty + $row3['m_qty'] ;
                $tgross = $tgross + $row3['m_grossweight'] ;
                ?>
                <tr>
                    <td><?php echo $i; ?></td>
                    <td><?php echo $row3['m_nodoc']; ?></td>
                    <td><?php echo $row3['m_namabarang']; ?></td>
                    <td><div align="right"><?php echo number_format($row3['m_qty'], 0, '.', ','); ?></div></td>
                    <td><div align="right"><?php echo number_format($row3['m_grossweight'], 2, '.', ','); ?></div></td>
                </tr>
                <?php
            }
        ?>
