<?php
	...
	$tsql = "exec dbo.sp_pos_all '".$tanggal1."','".$tanggal2."','".$kdcabang."','".$kdgroup."','".$kditem."','".$kdplu."','".$rubberid."','".$kodesupplier."','".$kdsupplier."','".$kddesigner."','".$kdtype."','".$kdcust."','".$kdnodoc."','".$kdby."' ";
...
require_once 'plugins/excel/PHPExcel.php';
	...
	$objPHPExcel->getActiveSheet()->setCellValue('N1', "Harga Barcode");
	$objPHPExcel->getActiveSheet()->setCellValue('O1', "Discount 1");
	$objPHPExcel->getActiveSheet()->setCellValue('P1', "Discount 2");
	$objPHPExcel->getActiveSheet()->setCellValue('Q1', "Discount 3");
	$objPHPExcel->getActiveSheet()->setCellValue('R1', "Discount 4");
	$objPHPExcel->getActiveSheet()->setCellValue('S1', "Harga Invoice");
	$objPHPExcel->getActiveSheet()->setCellValue('T1', "Berat");
	$objPHPExcel->getActiveSheet()->setCellValue('U1', "Butir");
	$objPHPExcel->getActiveSheet()->setCellValue('V1', "Carat");
	$objPHPExcel->getActiveSheet()->setCellValue('W1', "Harga M");
	$objPHPExcel->getActiveSheet()->setCellValue('X1', "Harga R");
	...
	do {
		while( $row = sqlsrv_fetch_array( $stmt ))
		{
			...
			$objPHPExcel->setActiveSheetIndex(0)
				 ...
				 ->setCellValue( "O" . $baris, $row['vf_discount1'])
				 ->setCellValue( "P" . $baris, $row['vf_discount2'])
				 ->setCellValue( "Q" . $baris, $row['vf_discount3'])
				 ->setCellValue( "R" . $baris, $row['vf_discount4'])
				 ->setCellValue( "S" . $baris, $row['vf_hargainv'])
				 ->setCellValue( "T" . $baris, $row['vf_grossweight'])
				 ->setCellValue( "U" . $baris, $row['vf_totbutir'])
				 ->setCellValue( "V" . $baris, $row['vf_totcarat'])
				 ->setCellValue( "W" . $baris, $row['vf_hargam'])
				 ->setCellValue( "X" . $baris, $row['vf_hargar']);
				 $baris++;;
