<?php
	...
	if ($tnomor == '')
	{
		...
		$tsqlnomor = "select max(right(m_nomor,4)) as nomormax from t_retursupp where m_cabang = '".$tcabang."' and year(m_tanggal) = ".$abc[2]." and month(m_tanggal) = ".$abc[1];
		...
		$tsql = "insert into t_retursupp (m_cabang, m_nomor, m_tanggal, m_kodesupp, m_nama, m_keterangan, m_status) 
				values('".$tcabang."','".$tnomor."','".$tgl."','".$tkodesupp."','".$tnama."','".$tket."','".$tstatus."')" ;
	...
	{
		$tketlog = 'EDIT';
		$tsql = "update t_retursupp set m_keterangan = '".$tket."' where m_cabang = '".$tcabang."' and m_nomor = '".$tnomor."'";
	}
	$stmt = sqlsrv_query( $con_dbnew, $tsql);
...
//Detail
		...
		if ($tnoplu != '')
		{
			if ( $new == 'Y' )
			{
				...
				if ($tqty >= $row_cekstock['m_qty'])
				{
					...
					//Insert table pos2
					$sql_insert = "insert into t_retursupp2
			...
			else
			{
				if ($hapus == 'on')
				{
					...
					//Hapus data transfer
					$sql_hapus = "delete from t_retursupp2 where m_cabang = '".$tcabang."' and m_nomor = '".$tnomor."' and m_kodebarang = '".$tkdbrg."' and m_productid = '".$tnoplu."' ";
					$stmt_hapus = sqlsrv_query( $con_dbnew, $sql_hapus);				
				}
				else
				{
					$sql_updatepos = "	update t_retursupp2 set m_keterangan = '".$tketerangan."'
	...
	header("Location: retursupp.php?st=".base64_encode($_POST['m_cabang'])."&pr=".base64_encode($_POST['periode'])."&nm=".base64_encode($tnomor)."&prm=".base64_encode($prm));
