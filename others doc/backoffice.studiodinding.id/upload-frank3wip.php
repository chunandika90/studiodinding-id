				...
				foreach($sheets as $sheet)
				{
					...
					while ( $i <= $countrow )
					{
						...
						*/
						...
						if ($productid <> '' )
						{
							$tsqlA = " 	insert into dbburungfrank.dbo.t_stockinvwip2 (m_cabang, m_kodebarang, m_lokasi, m_productid, m_qty, m_harga, m_status) 
										values ('".$cabang."','".$kodebarang."','".$lokasi."','".$productid."', ".$qty.", ".$harga.",'".$status."')";
							$stmtA = sqlsrv_query($con_dbnew, $tsqlA);
					
							$tsqlB = " 	insert into dbburungfrank.dbo.t_stockdatawip2 (m_kodebarang, m_productid, m_hpp, m_configid, m_rubberid, m_item, m_klasifikasi
