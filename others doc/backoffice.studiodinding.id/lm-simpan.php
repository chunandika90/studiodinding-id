...
//	echo $tsqlcust.'<br/>'  ;
	...
	if ($tnomor == '')
	{
		...
		$tsqlnomor = "select max(right(m_nomor,4)) as nomormax from t_pos where m_cabang = '".$tcabang."' and year(m_tanggal) = ".$abc[2]." and month(m_tanggal) = ".$abc[1]." and day(m_tanggal) = ".$abc[0];
		...
		$tnomor = 'LM'.substr($abc[2],-2).$abc[1].$abc[0].substr('00'.$nomax,-2) ;
		...
		$tsql = "insert into t_pos (m_cabang, m_nomor, m_tanggal, m_kodecust, m_nama, m_keterangan, m_status, m_kodesales, m_kodesales2, m_type) 
				values('".$tcabang."','".$tnomor."','".$tgl."','".$tkodecust."','".$tnama."','".$tket."','".$tstatus."','".$tsales."','".$tsales2."','L')" ;
	}
	else
	{
		$tketlog = 'EDIT';
				...
				if ($tqty >= $row_cekstock['m_qty'])
				{
					...
					$sql_insert = "insert into t_pos2
									(m_cabang, m_nomor, m_kodebarang, m_productid, m_lokasi, m_qty, m_harga, m_disc, m_discount, m_discount2, m_discount3, m_discount4, m_kurs, m_rate, m_keterangan, m_status)
									 values('".$tcabang."','".$tnomor."','".$tkdbrg."','".$tnoplu."','".$tlokasi."',".$tqty.",".$tharga.",0 ,0 ,0 ,0 ,0 ,'IDR',1,'','0')";
