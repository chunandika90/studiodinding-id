...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <div class="container" style="width: auto; padding: 0 20px;">
        ...
        <div class="container input-append" style="width: auto; padding: 0 10px;">
            ...
            <button class="btn" onClick="oc_purchase('<?php echo $prm; ?>','')">Search</button>
            <?php
			if (substr($xparam[3],0,1) == 'Y')
			{
				?>
            	<button class="btn" onClick="edit_data('<?php echo $prm; ?>','','')">New Receive</button>
				<?php
			}
			?>
        </div>
    ...
    <script type="text/javascript">
		function oc_purchase(vparam,vnomor)
		{
			...
			$.get('purchaselm-list.php',data,fungsi);
		}
		...
		function oc_detail(vparam,kdcab,nomor)
		{
			...
			$.get('purchaselm-view.php',data,fungsi);
		}

		function add_inv(vparam,kdcab,nomor)
		{
			var data={cb:kdcab,nm:nomor,pr:$('#periode').val()};
			var fungsi=function(respon){
					$("#viewdata").html(respon);
					oc_detail(kdcab,nomor);
				};
			$.get('purchaselm-invoice.php',data,fungsi);
		...
		function hapus_data(vparam,kdcab,nomor)
		{
			...
			$.get('purchaselm-hapus.php',data,fungsi);
		...
		function add_data(vparam,kdcab,nomor,kdbrg,productid)
		{
			var st = $('#kdstore').val() ;
			var pr = $('#periode').val() ;
			if (kdcab == '') { kdcab = st ; }
			window.open("purchaselm-add.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&st='+base64_encode(st)+'&pr='+base64_encode(pr)+'&prm='+base64_encode(vparam)+'&kd='+base64_encode(kdbrg)+'&pl='+base64_encode(productid),'_self');
		}
		...
		function edit_data(vparam,kdcab,nomor)
		{
			...
			var pr = $('#periode').val() ;
			if (kdcab == '') { kdcab = st ; }
			window.open("purchaselm-edit.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&st='+base64_encode(st)+'&pr='+base64_encode(pr)+'&prm='+base64_encode(vparam),'_self');
		}

		function print_all(kdcab,nomor)
		{
			window.open("purchaselm-printall.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor),'_blank');
		}

		function print_data(kdcab,nomor,kdbrg,productid)
		{
			window.open("purchaselm-print.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&kdbrg='+base64_encode(kdbrg)+'&productid='+base64_encode(productid),'_blank');
		}
