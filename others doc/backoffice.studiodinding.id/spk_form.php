    ...
    <div class="container" style="width: auto; padding: 0 20px;">
        ...
        <div class="span3 input-prepend">
            ...
            <select name="periode" id="periode" class="input-medium">
                <?php
				$tsqlbulan = "select distinct LEFT(convert(varchar(10),m_tanggal,120),7) as co_periode from t_spk where m_status = 'A'" ;
        ...
        <div class="container input-append" style="width: auto; padding: 0 10px;">
			...
			{
				?>
            	<button class="btn" onClick="edit_data('<?php echo $prm; ?>','<?php echo $kdcabang; ?>','')">Input SPK</button>
				<?php
			}
			?>
        </div>
   ...
   <div id="edit_spec" class="modal hide fade"  style="wid"tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <script type="text/javascript">
		...
		function oc_ttb(vparam,vnomor)
		{
			...
			$.get('spk_form-list.php',data,fungsi);
		}
		...
		function oc_detail(vparam,kdcab,nomor)
		{
			...
			$.get('spk_form-view.php',data,fungsi);
		}
		...
		{
			var pr = $('#periode').val() ;
			window.open("spk_form-hapus.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&pr='+base64_encode(pr)+'&prm='+base64_encode(vparam),'_self');
		}
		
		function approve_data(vparam,kdcab,nomor)
		{
			var pr = $('#periode').val() ;
			window.open("spk_form-approve.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&pr='+base64_encode(pr)+'&prm='+base64_encode(vparam),'_self');
		}
		...
		{
			var pr = $('#periode').val() ;
			window.open("spk_form-edit.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&pr='+base64_encode(pr)+'&prm='+base64_encode(vparam),'_self');
		}

		function print_all(kdcab,nomor)
		{
			window.open("spk_form-printall.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor),'_blank');
		}

		function print_data(kdcab,nomor,kdbrg,productid)
		{
			window.open("spk_form-print.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&kdbrg='+base64_encode(kdbrg)+'&productid='+base64_encode(productid),'_blank');
		}
		...
		function addRowToTable(aed)
		{
				...
				cellcolour.valign="top";
		  		cellcolour.innerHTML='<td><div align="left"><input class="input-mini" type="text" id="m_colour'+iteration+'" name="m_colour'+iteration+'" value="" style="text-align:right" /></div></td>';
