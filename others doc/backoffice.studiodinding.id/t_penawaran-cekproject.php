...
<div id="dialog-listcust" title="Daftar Project">
  <div class="dialog-content-scroll">
    <table class="table table-bordered table-striped table-hover table-condensed">
        <thead>
            <tr>
                <th width="20%">Nama Project 4</th>
                <th width="20%">Client</th>
                <th width="20%">Lokasi</th>
                <th width="30%">Alamat</th>
                <th width="30%">Supervisor Project</th>
            </tr>
        </thead>
        <tbody>
            <?php if ($stmt && $stmt->num_rows > 0): ?>
                <?php while ($row = $stmt->fetch_assoc()): ?>
                    <tr>
                        <td onClick="selectcust('<?php echo $row['m_kode']; ?>','<?php echo $row['nama_project']; ?>','<?php echo htmlspecialchars(json_encode($row["m_alamat"])); ?>','<?php echo $row['m_lokasi']; ?>','<?php echo $row['nama_client']; ?>','<?php echo $row['supervisor_project']; ?>')" style="cursor:pointer">
