...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <div class="container" style="width: auto; padding: 0 20px;">
        ...
        <div class="span3 input-prepend">
            ...
            <select name="periode" id="periode" class="input-medium">
                <?php
				$tsqlbulan = "select distinct LEFT(convert(varchar(10),m_tanggal,120),7) as co_periode from t_transfer where m_status = 'A'" ;
        ...
        <div class="container input-append" style="width: auto; padding: 0 10px;">
            ...
            <button class="btn" onClick="oc_transfer('<?php echo $prm ; ?>')">Search</button>
            <?php
			if (substr($xparam[3],0,1) == 'Y')
			{
				?>
	            <button class="btn" onClick="edit_data('<?php echo $prm ; ?>','','')">New Transfer</button>
                <?php
			}
			?>
    ...
    <div class="container" style="width: auto; padding: 0 20px;">
        ...
        <div class="container pull-right" style="width:auto; padding: 0 10px;">
            <span id="detaildata">
            </span>
        </div>
	</div>
    ...
    <script type="text/javascript">
		function oc_transfer(vparam)
		{
			var data={cb:$('#kdstore').val(),pr:$('#periode').val(),by:$('#searchby').val(),tx:$('#inputText').val(),prm:vparam};
			var fungsi=function(respon){
					$("#listdata").html(respon);
				};
			$.get('lmtransfer-list.php',data,fungsi);
		}

		function oc_detail(vparam,kdcab,nomor)
		{
			var data={kdlok:kdcab,nm:nomor,prm:vparam};
			var fungsi=function(respon){
					$("#detaildata").html(respon);
				};
			$.get('lmtransfer-view.php',data,fungsi);
		}
		...
		function edit_data(vparam,kdcab,nomor)
		{
			...
			var pr = $('#periode').val() ;
			if (kdcab == '') { kdcab = st ; }
			window.open("lmtransfer-edit.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&st='+base64_encode(st)+'&pr='+base64_encode(pr)+'&prm='+base64_encode(vparam),'_self');
		}

		function print_data(kdcab,nomor)
		{
			window.open("lmtransfer-print.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor),'_blank');
		}
		...
		function hapus_data(vparam,kdcab,nomor)
		{
			var st = $('#kdstore').val() ;
			var pr = $('#periode').val() ;
			window.open("lmtransfer-hapus.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&st='+base64_encode(st)+'&pr='+base64_encode(pr)+'&prm='+base64_encode(vparam),'_self');
		}
