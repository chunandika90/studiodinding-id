<?php
	...
	$tsql = "select a.*, DATE_FORMAT(a.m_tanggal, '%d/%m/%Y') AS co_tgl, DATE_FORMAT(a.m_tanggal_kirim, '%d/%m/%Y') AS m_tanggal_kirim , 
                    b.supervisor_project as m_nama_supervisor, b.nama_project as m_nama_project, 
                    b.m_lokasi, b.m_alamat m_alamat_project, b.nama_client, b.m_telepon_spv,
                    ...
                    case when a.m_jumlah_qty = a.m_terima_qty then 'Complete' else 'Not Complete' end m_status, d.m_nama m_payment_term
			 from t_po a
			 left join master_term_pembayaran d on a.m_payment_term = d.m_kode
	...
	if ($row['m_ppn_persen'] <> 0)
	{
		$dpp = ($row['m_jumlah_rp'] - $row['m_diskon_jumlah'])*11 / $row['m_ppn_persen'];
	}
	else
	{
		$dpp = 0;
    ...
    <td>
      ...
      <div class="info-container">
        ...
        <table class="info-table">
          ...
          <tr><td class="label">Ref Quot. No</td><td><?php echo $row['m_keterangan']; ?></td></tr>
        </table>
        ...
        <table class="info-table">
          ...
          <tr><td class="label">Delivery Date</td><td><?php echo $row['m_tanggal_kirim']; ?></td></tr>
      ...
      <table class="detail-table">
        ...
        $tdiskonrp = 0;
        $takhir = 0;
        $tsql2 = "SELECT a.*, c.m_nama AS co_namabarang , date(b.m_tanggal_kirim) m_tanggal_kirim
                  FROM t_po2 a 
				  left join t_penawaran b on a.m_nomor_request = b.m_nomor
        ...
        ?>
          <tr>
			...
			if ($row2['m_item'] <> 'M0331' && $row2['m_item'] <> 'M0354')
			{
				
			?>
				<td><?php
						echo $row2['co_namabarang'] . "</br>" . nl2br(htmlspecialchars($row2['m_keterangan']));
						
						$tanggal = isset($row2['m_tanggal_kirim']) ? trim($row2['m_tanggal_kirim']) : '';

							// filter semua kemungkinan kosong / null / default
							if (!in_array(strtolower($tanggal), ['', '0000-00-00', 'null'], true)) {
								// format ke dd/mm/yyyy
								$tgl_format = date('d/m/Y', strtotime($tanggal));
								echo "</br>Delivery date 2 = " . $tgl_format;
							}
					?> 
				</td>
			...
			{
			?>
				<td><?php echo nl2br(htmlspecialchars($row2['m_keterangan'])) ."</br> Delivery date = ". $row2['m_tanggal_kirim'] ; ?>  </td>
			...
			<td align="center">
				<?php
					$qty = $row2['m_qty']; // decimal(16,3) dari DB

					// format decimal, hapus trailing 0 desimal
					$formatted = number_format($qty, 3, ',', '.'); // selalu 3 desimal
					// hapus trailing 0 di belakang koma
		  ...
		  <tr>
			...
			<td class="bottom-left" style="width:65%; vertical-align:top;">
			  <table class="note-table">
				<tr>
				  ...
				  <td><?php echo $row['m_payment_term']; ?></td>
				</tr>
				<tr>
				  <td>Deliver To</td>
				...
				<tr>
				  ...
				  <td><?php echo $row['m_nama_supervisor'] ." ( ". $row['m_telepon_spv'] ." )"; ?></td>
				</tr>
			  </table>
			</td>
			...
			<td class="bottom-right" style="width:35%; vertical-align:top;">
			  <table class="total-table">
				...
				<tr>
				  <td class="label">Diskon RP</td>
				  <td class="amount">
					<div style="display:flex; justify-content:space-between; width:100%;">
					  <span>Rp</span>
					  <span><?php echo number_format($row['m_diskon2_jumlah'] ?? 0,0,',','.'); ?></span>
					</div>
				  </td>
				</tr>
				<tr>
				  ...
				  <td class="amount">
					<div style="display:flex; justify-content:space-between; width:100%;">
					  <span>Rp</span>
					  <span><?php echo number_format($dpp,0,',','.'); ?></span>
					</div>
				  </td>
				</tr>
				<tr>
