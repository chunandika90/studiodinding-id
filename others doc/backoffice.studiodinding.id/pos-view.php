<?php
   ...
   include "mssql-dbnew.php" ;
	...
	$tsql = "select a.*, convert(varchar(10),a.m_tanggal,103) as co_tgl, convert(varchar(10),a.m_canceldate,103) as co_tglbatal, b.m_alamat, b.m_kota, b.m_telepon1, b.m_telepon2 from t_pos a, mscustomer b where a.m_cabang = '".$kdcabang."' and a.m_nomor = '".$nomor."' and a.m_kodecust = b.m_kode " ;
	$stmt = sqlsrv_query( $con_dbnew, $tsql);
	$row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ;

	// Cek total penjualan
	$tsqljual = "select isnull(sum(( m_qty * m_harga ) - m_discount - m_discount2 - m_discount3 - m_discount4),0) as cojual from t_pos2 where m_cabang = '".$kdcabang."' and m_nomor = '".$nomor."'";
	...
	// Cek total bayat
	$tsqlbyr = "select isnull(sum(m_jumlah),0) as cobayar from t_pos3 where m_cabang = '".$kdcabang."' and m_nomor = '".$nomor."'";
...
?><table class="table table-bordered table-condensed">
    <tbody>
        ...
        <tr>
            <td><b>Jumlah USD</b></td>
            <td><b><?php echo number_format($row['m_jumlah'], 0, '.', ',') ?></b></td>
            <td><b>Rate</b></td>
            <td><b><?php echo number_format($row['m_rate'], 0, '.', ',') ?></b></td>
        </tr>
        <tr>
            <td><b>Terbayar USD</b></td>
            <td><b><?php echo number_format($row['m_bayar'], 0, '.', ',') ?></b></td>
            <td><b>Terbayar RP</b></td>
            <td><b><?php echo number_format($row['m_bayar_rp'], 0, '.', ',') ?></b></td>
        </tr>
...
<table class="table table-bordered table-striped table-hover table-condensed">
    <thead>
        <tr>
            ...
            <th width="6%"><div align="right">Rate</div></th>
            <th width="6%">Total RP</th>
        ...
        <?php
            ...
            while( $row3 = sqlsrv_fetch_array( $stmt3, SQLSRV_FETCH_ASSOC))
            {
                ...
                <tr>
                    ...
                    <td><div align="right"><?php echo number_format($row3['m_rate'], 0, '.', ','); ?></div></td>
                    <td><div align="right"><?php echo number_format($row3['m_total'], 0, '.', ','); ?></div></td>
    ...
    <tfoot>
        <tr>
            ...
            <th></th>
            <th><div align="right"><?php echo number_format($ttotal, 0, '.', ',') ; ?></div></th>
            <th></th>
        </tr>
        <tr>
			...
			if(($row['m_status'] != 'B') && ( substr($xparam[3],1,1) == 'Y'))
			{
				...
				{				
				?>
                      <button class="btn btn-primary" onclick="add_inv('<?php echo $prm; ?>','<?php echo $kdcabang; ?>','<?php echo $nomor; ?>')">Entry Pembayaran</button>
                <?php
				}
			}
			else
