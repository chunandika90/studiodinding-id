...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <script type="text/javascript">
		...
		function add_inv(vparam, kdcab,nomor)
		{
			...
			$.get('pos-invoice.php',data,fungsi);
		...
		function edit_data(vparam, kdcab,nomor)
		{
			...
			var pr = $('#periode').val() ;
			if (kdcab == '') { kdcab = st ; }
			window.open("pos-edit.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&st='+base64_encode(st)+'&pr='+base64_encode(pr)+'&prm='+base64_encode(vparam),'_self');
		}
		...
		function print_data(kdcab,nomor,kdbrg,productid)
		{
			...
			if (kdbrg == 'P0000004')
			{
				if (confirm('Cetak Gambar Product ?'))
				{
					cetakpic = 'Y';
				}
				else
			...
			window.open("pos-print.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&kdbrg='+base64_encode(kdbrg)+'&productid='+base64_encode(productid)+'&p='+cetakpic,'_blank');
		}
