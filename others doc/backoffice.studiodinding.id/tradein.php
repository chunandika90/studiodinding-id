...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <div class="container" style="width: auto; padding: 0 20px;">
        ...
        <div class="span3 input-prepend">
            ...
            <select name="periode" id="periode" class="input-medium">
                <?php
				$tsqlbulan = "select distinct LEFT(convert(varchar(10),m_tanggal,120),7) as co_periode from t_pos where m_status = 'A' and m_type = 'T'" ;
        ...
        <div class="container input-append" style="width: auto; padding: 0 10px;">
            ...
            <button class="btn" onClick="edit_data('<?php echo $prm; ?>','','')">New Tradein</button>
    ...
    <div class="container" style="width: auto; padding: 0 20px;">
        ...
        <div class="container pull-right" style="width:70%; padding: 0 10px;">
            <span id="detaildata">
            </span>
        </div>
	</div>
    ...
    <script type="text/javascript">
		function oc_pos(vparam, vnomor)
		{
			...
			$.get('tradein-list.php',data,fungsi);
		}
		...
		function oc_detail(vparam, kdcab,nomor)
		{
			...
			$.get('tradein-view.php',data,fungsi);
		}
		...
		function add_inv(vparam, kdcab,nomor)
		{
			...
			$.get('tradein-invoice.php',data,fungsi);
		...
		function batal_pos(vparam, kdcab,nomor)
		{
			...
			$.get('tradein-batal.php',data,fungsi);
		...
		function edit_data(vparam, kdcab,nomor)
		{
			...
			var pr = $('#periode').val() ;
			if (kdcab == '') { kdcab = st ; }
			window.open("tradein-edit.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&st='+base64_encode(st)+'&pr='+base64_encode(pr)+'&prm='+base64_encode(vparam),'_self');
		}

		function print_all(kdcab,nomor)
		{
			window.open("pos-printall.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor),'_blank');
		}

		function print_data(kdcab,nomor,kdbrg,productid)
		{
			window.open("pos-print.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&kdbrg='+base64_encode(kdbrg)+'&productid='+base64_encode(productid),'_blank');
		}

		function print_exch(kdcab,nomor)
		{
			window.open("tradein-printall.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor),'_blank');
		}

		function print_cert(kdcab,nomor,kdbrg,productid)
		{
			
			window.open("pos-sertifikat.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&kdbrg='+base64_encode(kdbrg)+'&productid='+base64_encode(productid),'_blank');
		}
	</script>
