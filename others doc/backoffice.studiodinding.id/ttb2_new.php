...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        <title>Penerimaan Internal</title>
    ...
    <div class="container" style="width: auto; padding: 0 20px;">
        ...
        <div class="container input-append" style="width: auto; padding: 0 10px;">
            ...
            <select name="searchby" id="searchby" class="input-medium">
                <option value="m_nomor" >Nomor TTB</option>
                <option value="m_rubberid" selected>Kode Karet</option>
                <option value="m_productid">Kode Plu</option>
            </select>
   ...
   <div id="edit_spec" class="modal hide fade"  style="wid"tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <script type="text/javascript">
		...
		{
			var pr = $('#periode').val() ;
			window.open("ttb2-hapus.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&pr='+base64_encode(pr)+'&prm='+base64_encode(vparam),'_self');
		}
		...
		{
			var pr = $('#periode').val() ;
			window.open("ttb2_new-edit.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&pr='+base64_encode(pr)+'&prm='+base64_encode(vparam),'_self');
		}

		function print_all(kdcab,nomor)
		{
			window.open("ttb2-printall.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor),'_blank');
		}

		function print_data(kdcab,nomor,kdbrg,productid)
		{
			window.open("ttb2-print.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&kdbrg='+base64_encode(kdbrg)+'&productid='+base64_encode(productid),'_blank');
		}
