...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <div class="container" style="width: auto; padding: 0 20px;">
        ...
        <div class="span3 input-prepend">
            ...
            <select name="periode" id="periode" class="input-medium">
                <?php
				$tsqlbulan = "select distinct LEFT(convert(varchar(10),m_tanggal,120),7) as co_periode from t_retur where m_status = 'A'" ;
        ...
        <div class="container input-append" style="width: auto; padding: 0 10px;">
            ...
            <button class="btn" onClick="oc_retur('<?php echo $prm; ?>','')">Search</button>
            <?php
			if (substr($xparam[3],0,1) == 'Y')
			{
				?>
            	<button class="btn" onClick="edit_data('<?php echo $prm; ?>','','')">New Retur</button>
				<?php
			}
			?>
        </div>
    ...
    <script type="text/javascript">
		function oc_retur(vparam,vnomor)
		{
			...
			$.get('retur-list.php',data,fungsi);
		}
		...
		function oc_detail(vparam,kdcab,nomor)
		{
			...
			$.get('retur-view.php',data,fungsi);
		}
		...
		function add_inv(vparam,kdcab,nomor)
		{
			...
			$.get('retur-invoice.php',data,fungsi);
		...
		function hapus_data(vparam,kdcab,nomor)
		{
			...
			$.get('retur-hapus.php',data,fungsi);
		...
		function edit_data(vparam,kdcab,nomor)
		{
			...
			var pr = $('#periode').val() ;
			if (kdcab == '') { kdcab = st ; }
			window.open("retur-edit.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&st='+base64_encode(st)+'&pr='+base64_encode(pr)+'&prm='+base64_encode(vparam),'_self');
		}

		function print_all(kdcab,nomor)
		{
			window.open("retur-printall.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor),'_blank');
		}

		function print_data(kdcab,nomor,kdbrg,productid)
		{
			window.open("retur-print.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&kdbrg='+base64_encode(kdbrg)+'&productid='+base64_encode(productid),'_blank');
		}
