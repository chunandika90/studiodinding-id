...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <script type="text/javascript">
		function oc_pos(vparam,vnomor)
		{
			...
			$.get('titipan-list.php',data,fungsi);
		}
		...
		function oc_detail(vparam,kdcab,nomor)
		{
			...
			$.get('titipan-view.php',data,fungsi);
		}
		...
		{
			var pr = $('#periode').val() ;
			window.open("titipan-edit.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&pr='+base64_encode(pr)+'&prm='+base64_encode(vparam),'_self');
		}


		function print_data(kdcab,nomor)
		{
			window.open("titipan-print.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor),'_blank');
		}
		...
		function batal_pos(vparam, kdcab,nomor)
		{
			...
			$.get('titipan-batal.php',data,fungsi);
			
			$('#view_modal').modal();
		}
		
		function kembali_pos(vparam, kdcab,nomor)
		{
			var data={cb:kdcab,nm:nomor,pr:$('#periode').val(),prm:vparam};
			var fungsi=function(respon){
					$("#viewdata").html(respon);
			...
			$.get('titipan-kembali.php',data,fungsi);
