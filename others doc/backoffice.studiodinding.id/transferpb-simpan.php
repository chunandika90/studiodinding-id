<?php
	...
	if ($tnomor == '')
	{
		...
		$tnomor = 'PB'.substr($abc[2],-2).$abc[1].substr('0000'.$nomax,-4) ;
		
		$tsql = "insert into t_transferpb (m_cabang, m_nomor,m_tanggal, m_spk, m_tukang, m_keterangan, m_status, m_cabang2) 
				values('".$tlokasi."','".$tnomor."','".$tgl."','".$tspk."','".$ttukang."','".$tket."','".$status."','".$tlokasi2."')" ;
	}
	else
	{
		$tketlog = 'EDIT';
		$tsql = "update t_transferpb set m_keterangan = '".$tket."', m_spk = '".$tspk."', m_tukang = '".$ttukang."'
		...
		if ($size != '')
		{
			if ( $hapus != 'on' )
			{
				...
				//insert ke table ttb3
					$sql_ttb   = "insert into t_transferpb2 (m_nomor, m_no, m_shape, m_size, m_dimensi, m_dimensi2, m_dimensi3, m_gia, m_butir, m_carat, m_cabang, m_cabang2)  
								 values('".$tnomor."','".$no."','".$shape."','".$size."','".$dimensi."','".$dimensi2."','".$dimensi3."','".$gia."',".$butir.",".$carat.",'".$tlokasi."','".$tlokasi2."')" ;
					
				}
				else
				{
					$sql_ttb =  "update t_transferpb2 set m_carat = ".$carat." ,m_butir = ".$butir." ,m_dimensi = '".$dimensi."',
			...
			{
					$sql_ttb = "delete t_transferpb2  where m_nomor = '".$tnomor."'and m_shape = '".$shape."' and m_size = '".$size."' and m_no = '".$no."' ";
	...
	header("Location: transferpb.php?st=".base64_encode($_POST['m_cabang'])."&pr=".base64_encode($_POST['periode'])."&nm=".base64_encode($tnomor)."&prm=".base64_encode($prm));
