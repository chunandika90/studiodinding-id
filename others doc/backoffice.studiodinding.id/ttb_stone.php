    ...
    <div class="container" style="width: auto; padding: 0 20px;">
		<div class="span3 input-prepend">
            ...
            <select name="kdcabang" id="kdcabang" class="input-large" >
                <?php
				$tsqlstore = "select m_kode, m_nama from mslokasi  order by m_kode asc" ;
        ...
        <div class="span3 input-prepend">
            ...
            <select name="periode" id="periode" class="input-medium">
                <?php
				$tsqlbulan = "select distinct LEFT(convert(varchar(10),m_tanggal,120),7) as co_periode from t_ttb_stone where m_status = 'A'" ;
   ...
   <div id="edit_spec" class="modal hide fade"  style="wid"tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <script type="text/javascript">
		...
		function oc_ttb(vparam,vnomor)
		{
			...
			$.get('ttb_stone-list.php',data,fungsi);
		}
		...
		function oc_detail(vparam,kdcab,nomor)
		{
			...
			$.get('ttb_stone-view.php',data,fungsi);
		}
		...
		{
			var pr = $('#periode').val() ;
			window.open("ttb_stone-hapus.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&pr='+base64_encode(pr)+'&prm='+base64_encode(vparam),'_self');
		}
		...
		{
			var pr = $('#periode').val() ;
			window.open("ttb_stone-edit.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&pr='+base64_encode(pr)+'&prm='+base64_encode(vparam),'_self');
		}

		function print_all(kdcab,nomor)
		{
			window.open("ttb_stone-printall.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor),'_blank');
		}

		function print_data(kdcab,nomor,kdbrg,productid)
		{
			window.open("ttb_stone-print.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&kdbrg='+base64_encode(kdbrg)+'&productid='+base64_encode(productid),'_blank');
		}
