...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <div class="container" style="width: auto; padding: 0 20px;">
        ...
        <div class="span1">
			...
			{
				?>
                <img src="images/printer.gif" style="cursor:pointer" id="cetakreport1c" onclick="cetak1b('<?php echo $tgl1; ?>','<?php echo $tgl2; ?>','<?php echo $kdcabang; ?>','<?php echo $kdgroup; ?>','<?php echo $kdkatg; ?>','<?php echo $kditem; ?>','<?php echo $kdsales; ?>','<?php echo $kdby; ?>') "/> 
                <img src="images/excel.gif" style="cursor:pointer" id="excelreport1c" onclick="exel1b('<?php echo $tgl1; ?>','<?php echo $tgl2; ?>','<?php echo $kdcabang; ?>','<?php echo $kdgroup; ?>','<?php echo $kdkatg; ?>','<?php echo $kditem; ?>','<?php echo $kdsales; ?>','<?php echo $kdby; ?>') "/>  
            	<?php
			}
			?>
        </div>
        ...
        <table id="table1" class="controller table table-bordered table-striped table-hover table-condensed">
        	<thead>
                <tr data-level="header" class="header">
                    <th width="20%"><div align="center">Report Resell</div></th>
            ...
            <tbody>
			<?php
            $tsql = "select * from dbo.f_reportresell('".$tanggal1."', '".$tanggal2."', '".$kdcabang."', '".$kdgroup."', '".$kdkatg."', '".$kditem."')" ;
...
//			echo $tsql ;
            ...
            {	
                ...
                <tr <?php echo $tampilanfont; ?> data-level="<?php echo $row['vf_level']; ?>" id="rowke<?php echo $i; ?>" height="25px">
                    ...
                    <td>
                    	<div align="center">
                        <img src="images/view_icon.gif" style="cursor:pointer" id="viewdetail" onClick="oc_detail('<?php echo $prm; ?>','<?php echo $tanggal1; ?>','<?php echo $tanggal2; ?>','<?php echo $row['vf_idcab']; ?>','<?php echo $row['vf_idgroup']; ?>','<?php echo $row['vf_idkatg']; ?>','<?php echo $row['vf_iditem']; ?>')"/>
                        </div>
                    </td>
                </tr>
                <?php
    ...
    <script type="text/javascript">
		...
		function oc_report(vparam)
		{
			...
			window.open("report-resell.php?tg1="+base64_encode(tgl1)+"&tg2="+base64_encode(tgl2)+"&cb="+base64_encode(data)+'&gr='+base64_encode(kdgroup)+'&kt='+base64_encode(kdkatg)+'&it='+base64_encode(kditem)+'&prm='+base64_encode(vparam),'_self');
		}

		function oc_detail(vparam,tgl1,tgl2,kdcab,kdgroup,kdkatg,kditem)
		{
			var data={tg1:tgl1,tg2:tgl2,cb:kdcab,gr:kdgroup,kt:kdkatg,it:kditem,prm:vparam};
			var fungsi=function(respon){
					$("#listplu").html(respon);
				};
			$.get('report-resell2.php',data,fungsi);
		}
		...
		{	
			window.open('report-resellp.php?tg1='+tgl1+'&tg2='+tgl2+'&cb='+kdcab+'&gr='+kdgroup+'&kt='+kdkatg+'&it='+kditem+'&sl='+kdsales+'&by='+reportby,'_blank');
		}
		
		function exel1b (tgl1,tgl2,kdcab,kdgroup,kdkatg,kditem,kdsales,reportby)
		{	
			window.open('report-resellx.php?tg1='+tgl1+'&tg2='+tgl2+'&cb='+kdcab+'&gr='+kdgroup+'&kt='+kdkatg+'&it='+kditem+'&sl='+kdsales+'&by='+reportby,'_blank');
		}
		
		function cetak1c (tgl1,tgl2,kdcab,kdgroup,kdkatg,kditem)
		{	
			window.open('report-resell2p.php?tg1='+tgl1+'&tg2='+tgl2+'&cb='+kdcab+'&gr='+kdgroup+'&kt='+kdkatg+'&it='+kditem,'_blank');
		}
		
		function exel1c (tgl1,tgl2,kdcab,kdgroup,kdkatg,kditem)
		{	
			window.open('report-resell2x.php?tg1='+tgl1+'&tg2='+tgl2+'&cb='+kdcab+'&gr='+kdgroup+'&kt='+kdkatg+'&it='+kditem,'_blank');
		}
	</script>
