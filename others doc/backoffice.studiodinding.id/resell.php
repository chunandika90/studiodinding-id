    ...
    <div class="container" style="width: auto; padding: 0 20px;">
        ...
        <div class="span3 input-prepend">
            ...
            <select name="periode" id="periode" class="input-medium">
                <?php
				$tsqlbulan = "select distinct LEFT(convert(varchar(10),m_tanggal,120),7) as co_periode from t_resell where m_status = 'A'" ;
        ...
        <div class="container input-append" style="width: auto; padding: 0 10px;">
            ...
            <button class="btn" onClick="edit_data('<?php echo $prm; ?>','','')">New Resell</button>
    ...
    <script type="text/javascript">
		function oc_pos(vparam, vnomor)
		{
			...
			var fungsi=function(respon){
					...
					{
						oc_detail(kdcab,vnomor);
					}
				};
			$.get('resell-list.php',data,fungsi);
		}
		...
		function oc_detail(vparam, kdcab,nomor)
		{
			...
			$.get('resell-view.php',data,fungsi);
		}

		function batal_resell(vparam, kdcab,nomor)
		{
			...
			$.get('resell-batal.php',data,fungsi);
		...
		function edit_data(vparam, kdcab,nomor)
		{
			...
			var pr = $('#periode').val() ;
			if (kdcab == '') { kdcab = st ; }
			window.open("resell-edit.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&st='+base64_encode(st)+'&pr='+base64_encode(pr)+'&prm='+base64_encode(vparam),'_self');
		}

		function print_resell(kdcab,nomor)
		{
			window.open("resell-printall.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor),'_blank');
		}
