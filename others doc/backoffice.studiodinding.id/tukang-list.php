<?php
	...
	$tsql = "select a.*, convert(varchar(10),a.m_tanggal,103) as co_tgl, convert(varchar(10),a.m_tgl_terima,103) as co_tgl_terima
			from t_tukang a where year(a.m_tanggal) = ".$dumb[0]." and month(a.m_tanggal) = ".$dumb[1] ;
...
<table class="table table-bordered table-striped table-hover table-condensed">
    <thead>
        <tr>
            <th colspan="5"><h4>Data Kirim Bahan</h4></th>
        </tr>
        <tr>
            <th width="100">Nomor</th>
            <th width="100">Tanggal Kirim</th>
            <th width="120">Nama Tukang</th>
            <th width="120">Tanggal Terima</th>
        </tr>
    </thead>
    <tbody>
        <?php
            while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC))
            {
                ...
                <tr <?php if($row['m_status']== 'B'){ ?> style="color:#F00" <?php } ?> >
                    ...
                    <td><?php echo $row['co_tgl_terima']; ?></td>
                </tr>
                <?php
            }
            ?>
