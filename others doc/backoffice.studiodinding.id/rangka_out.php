...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <script type="text/javascript">
		function oc_pos(vparam,vnomor)
		{
			...
			$.get('rangka_out-list.php',data,fungsi);
		}
		...
		function oc_detail(vparam,kdcab,nomor)
		{
			...
			$.get('rangka_out-view.php',data,fungsi);
		}
		...
		{
			var pr = $('#periode').val() ;
			window.open("rangka_out-hapus.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&pr='+base64_encode(pr)+'&prm='+base64_encode(vparam),'_self');
		}

		function edit_data(vparam,kdcab,nomor)
		{
			var pr = $('#periode').val() ;
			window.open("rangka_out-edit.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor)+'&pr='+base64_encode(pr)+'&prm='+base64_encode(vparam),'_self');
		}

		function print_all(kdcab,nomor)
		{
			window.open("rangka_out-print.php?cb="+base64_encode(kdcab)+'&nm='+base64_encode(nomor),'_blank');
		}
